...
setup_harness(_Test, _Args) ->
    ...
    logger:info("Resetting nodes to fresh state"),
    rt:pmap(fun(Host) ->
...
run_riak(Node, Cmd) ->
    ...
    logger:info("Running: ~s :: ~s", [get_host(Node), Exec]),
    ssh_cmd(Node, Exec).
...
run_git(Host, Path, Cmd) ->
    ...
    logger:info("Running: ~s :: ~s", [Host, Exec]),
    ssh_cmd(Host, Exec).

remote_cmd(Node, Cmd) ->
    logger:info("Running: ~s :: ~s", [get_host(Node), Cmd]),
...
ssh_cmd(Node, Cmd, Return) ->
    case rt_config:get(rtssh_verbose, false) of
        true ->
            logger:info("Running: ~s :: ~s", [Node, Cmd]);
        false -> ok
    end,
...
update_app_config_file(Node, ConfigFile, Config, Current) ->
    logger:info("rtssh:update_app_config_file(~p, ~s, ~p)",
...
set_conf(all, NameValuePairs) ->
    logger:info("rtssh:set_conf(all, ~p)", [NameValuePairs]),
    Hosts = rt_config:get(rtssh_hosts),
    All = [{Host, DevPath} || Host <- Hosts,
                              DevPath <- devpaths()],
...
set_advanced_conf(all, NameValuePairs) ->
    logger:debug("rtssh:set_advanced_conf(all, ~p)", [NameValuePairs]),
    Hosts = rt_config:get(rtssh_hosts),
    All = [{Host, DevPath} || Host <- Hosts,
                              DevPath <- devpaths()],
    rt:pmap(fun({Host, DevPath}) ->
    ...
    receive
        ...
        {Port, {exit_status, Status}} ->
            case Status of
                ...
                _ ->
                    Cmd = get(Port),
                    logger:info("~p returned exit status: ~p",
                                [Cmd, Status]),
                    ok
            end,
            erase(Port),
    ...
    case wildcard(Host, DevPath ++ "/dev*") of
        ...
        Devs ->
            [begin
                 ...
                 logger:info("Stopping Node... ~s :: ~s ~~ ~s.",
                             [Host, Cmd, Status])
             end || D <- Devs]
    end,
    ok.
