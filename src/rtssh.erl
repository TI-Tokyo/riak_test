...
%% -------------------------------------------------------------------
%%
%% @deprecated Needs to be updated to be usable.
-module(rtssh).
...
setup_harness(_Test, _Args) ->
    ...
    %% [?LOG_DEBUG("R: ~0p", [wildcard(Host, "/tmp/*")]) || Host <- Hosts],
        ...
        ["advanced.config" | T] ->
            ...
            case Files of
                [] -> %% No file generated by chkconfig. this isn't great
                    ?LOG_ERROR("Cuttlefish Failure."),
                    ?LOG_INFO("chkconfig:"),
                    [ ?LOG_INFO("~s", [Line]) || Line <- ChkConfigOutput ],
                    ?assert(false);
                _ ->
                    File = hd(Files),
...
node_to_host(Node) ->
    case string:tokens(atom_to_list(Node), "@") of
        ["riak", Host] -> Host;
        _ ->
            throw(io_lib:format("rtssh:node_to_host couldn't figure out the host of ~0p", [Node]))
    end.

deploy_nodes(NodeConfig, Hosts) ->
    Path = relpath(root),
    ?LOG_INFO("Riak path: ~0p", [Path]),
    %% NumNodes = length(NodeConfig),
...
upgrade(Node, NewVersion, Config, _UpgradeCallback) ->
    ...
    ?LOG_INFO("Upgrading ~0p : ~0p -> ~0p", [Node, Version, NewVersion]),
    stop(Node),
...
run_riak(Node, Cmd) ->
    ...
    ?LOG_INFO("Running: ~s :: ~s", [get_host(Node), Exec]),
    ssh_cmd(Node, Exec).
...
run_git(Host, Path, Cmd) ->
    ...
    ?LOG_INFO("Running: ~s :: ~s", [Host, Exec]),
    ssh_cmd(Host, Exec).

remote_cmd(Node, Cmd) ->
    ?LOG_INFO("Running: ~s :: ~s", [get_host(Node), Cmd]),
...
remote_read_file(Node, File) ->
    ...
    case ssh_cmd(Node, "cat " ++ File) of
        {0, Text} ->
            %% io:format("~0p/~0p: read:~n~p~n", [Node, File, Text]),
...
do_update_vm_args(Node, Props) ->
    ...
    %% ?LOG_DEBUG("~0p", [iolist_to_binary(Output)]),
...
update_app_config_file(Node, ConfigFile, Config, Current) ->
    ?LOG_INFO("rtssh:update_app_config_file(~0p, ~s, ~0p)",
...
set_conf(all, NameValuePairs) ->
    ?LOG_INFO("rtssh:set_conf(all, ~0p)", [NameValuePairs]),
    Hosts = rt_config:get(rtssh_hosts),
    All = [{Host, DevPath} || Host <- Hosts,
                              DevPath <- devpaths()],
...
set_advanced_conf(all, NameValuePairs) ->
    ?LOG_DEBUG("rtssh:set_advanced_conf(all, ~0p)", [NameValuePairs]),
    Hosts = rt_config:get(rtssh_hosts),
    All = [{Host, DevPath} || Host <- Hosts,
                              DevPath <- devpaths()],
    rt:pmap(fun({Host, DevPath}) ->
...
all_the_files(Host, DevPath, File) ->
    case wildcard(Host, DevPath ++ "/dev/dev*/" ++ File) of
        error ->
            ...
            [];
        Files ->
            ?LOG_DEBUG("~s :: files: ~0p", [Host, Files]),
            Files
    end.
    ...
    receive
        ...
        {Port, {exit_status, Status}} ->
            case Status of
                ...
                _ ->
                    Cmd = get(Port),
                    ?LOG_INFO("~0p returned exit status: ~0p",
                               [Cmd, Status]),
                    ok
            end,
            erase(Port),
...
stop_all(Host, DevPath) ->
    case wildcard(Host, DevPath ++ "/dev*") of
        error ->
            ?LOG_INFO("~s is not a directory.", [DevPath]);
        Devs ->
            [begin
                 Cmd = D ++ "/bin/riak stop",
