...
confirm() ->
    ...
    logger:info("Deploy node to test command line"),
...
console_up_test(Node) ->
    logger:info("Node is already up, `riak console` should fail"),
...
console_test(Node) ->
    ...
    logger:info("Testing riak console on ~s", [Node]),
...
start_up_test(Node) ->
    ...
    logger:info("Testing riak start now will return 'already running'"),
...
start_test(Node) ->
    ...
    logger:info("Testing riak start works on ~s", [Node]),

    {ok, StartPass} = rt:riak(Node, ["start"]),
    logger:info("StartPass: ~p", [StartPass]),
...
ping_up_test(Node) ->
    ...
    logger:info("Testing riak ping on ~s", [Node]),

    %% ping / pong
    %% rt:start_and_wait(Node),
    logger:info("Node up, should ping"),
...
status_up_test(Node) ->
    logger:info("Test riak admin status on ~s", [Node]),
...
status_down_test(Node) ->
    logger:info("Test riak admin status while down"),
    {ok, {ExitCode, StatusOut}} = rt:admin(Node, ["status"], [return_exit_code]),
...
getpid_up_test(Node) ->
    logger:info("Test riak get pid on ~s", [Node]),
...
getpid_down_test(Node) ->
    logger:info("Test riak getpid fails on ~s", [Node]),
    {ok, PidOut} = rt:riak(Node, ["pid"]),
