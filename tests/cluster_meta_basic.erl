...
test_writes_after_partial_cluster_failure([N1 | _]=Nodes) ->
    logger:info("testing writes after partial cluster failure"),
    ...
    logger:info("stopping nodes: ~p remaining nodes: ~p", [StopNodes, AliveNodes]),
    [rt:stop(N) || N <- StopNodes],

    metadata_put(N1, ?PREFIX1, ?KEY1, ?VAL2),
    wait_until_metadata_value(AliveNodes, ?PREFIX1, ?KEY1, ?VAL2),

    logger:info("bring stopped nodes back up: ~p", [StopNodes]),
...
test_fold_full_prefix([N1 | _]=Nodes) ->
    ...
    logger:info("testing prefix (~p) fold on ~p", [?PREFIX2, N1]),
...
test_metadata_conflicts([N1, N2 | _]=Nodes) ->
    rt:load_modules_on_nodes([?MODULE], Nodes),
    logger:info("testing conflicting writes to a key"),
    ...
    logger:info("checking object count after resolve on get w/o put"),
    ...
    logger:info("checking object count after resolve on itr_key_values w/o put"),
    ?assertEqual(2, rpc:call(N1, ?MODULE, object_count, [?PREFIX1, ?KEY2])),
    ?assertEqual(2, rpc:call(N2, ?MODULE, object_count, [?PREFIX1, ?KEY2])),

    %% assert that we no longer have siblings when allow_put=true
    logger:info("checking object count afger resolve on get w/ put"),
...
wait_until_metadata_value(Node, Prefix, Key, Opts, Val) ->
    logger:info("wait until {~p, ~p} equals ~p on ~p", [Prefix, Key, Val, Node]),
    F = fun() ->
...
wait_until_object_count(Node, Prefix, Key, Count) ->
    logger:info("wait until {~p, ~p} has object count ~p on ~p", [Prefix, Key, Count, Node]),
    F = fun() ->
...
print_tree(Root, Nodes) ->
    ...
    logger:info("broadcast tree: ~p", [Tree]).
