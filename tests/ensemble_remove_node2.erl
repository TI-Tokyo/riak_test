...
confirm() ->
    ...
    logger:info("Removing Nodes 2, 3 and 4 from the cluster"),
    rt:leave(Node2),
    ok = ensemble_util:wait_until_stable(Node, NVal),
    rt:leave(Node3),
    ok = ensemble_util:wait_until_stable(Node, NVal),
    ...
    logger:info("Need to wait at least a minute to confirm no exit"),
    timer:sleep(60000),

    %% Nodes should still be in leaving state
    logger:info("Getting ring from node ~w", [Node]),
    ...
    %% never exit
    logger:info("Reading From SC Bucket"),
    ...
    logger:info("Cluster ~w", [Cluster]),
    ?assertEqual(4, length(Cluster)),

    logger:info("Removing intercept and waiting until nodes shutdown"),
