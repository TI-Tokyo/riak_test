...
confirm() ->
    ...
    ?LOG_INFO("Removing Nodes 2, 3 and 4 from the cluster"),
    rt:leave(Node2),
    ok = ensemble_util:wait_until_stable(Node, NVal),
    rt:leave(Node3),
    ok = ensemble_util:wait_until_stable(Node, NVal),
    ...
    ?LOG_INFO("Need to wait at least a minute to confirm no exit"),
    timer:sleep(60000),

    %% Nodes should still be in leaving state
    ?LOG_INFO("Getting ring from node ~w", [Node]),
    ...
    %% never exit
    ?LOG_INFO("Reading From SC Bucket"),
    ...
    ?LOG_INFO("Cluster ~w", [Cluster]),
    ?assertEqual(4, length(Cluster)),

    ?LOG_INFO("Removing intercept and waiting until nodes shutdown"),
