...
confirm() ->
    ...
    ?LOG_INFO("Change ~0p handoff_ip from ~0p to ~0p",
    ...
    ?LOG_INFO("Write data to the cluster"),
    rt:systest_write(Node1, 100),

    ?LOG_INFO("Join ~0p to the cluster and wait for handoff to finish",
               [Node2]),
    rt:join(Node2, Node1),
    ...
    ?LOG_INFO("Change ~0p handoff_ip to \"0.0.0.0\"", [Node3]),
    ...
    ?LOG_INFO("Test gh_riak_core_176 passed"),
    pass.
