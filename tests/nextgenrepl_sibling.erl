...
test_repl_between_clusters(ClusterA, ClusterB, ClusterC) ->
    ...
    logger:info("Confirm that cluster C has no objects"),
    logger:info("Chaining real-time replication doesn't replicate"),
    logger:info("Real-time repl requires a mesh topology between clusters"),
    read_from_cluster(NodeC, 1, 1000, ?COMMMON_VAL_INIT, 1000),

    logger:info("Rounds of full-sync should replicate all, no more than 12"),
    FSCFun = 
        fun() ->
    ...
    logger:info("Create some siblings - if we update in A then in C"),
    ...
    logger:info("A should have siblings"),
    logger:info("B and C should have different versions but not siblings"),
    ...
    logger:info("All other objects undisturbed"),
    ...
    logger:info("Full sync from B to C should create siblings in C"),
    {clock_compare, 100}
    ...
    logger:info("Full sync from A to B should create siblings in B"),
    {clock_compare, 100}
        = fullsync_check({NodeA, IPA, PortA, ?A_NVAL},
                            {NodeB, IPB, PortB, ?B_NVAL},
                            cluster_b),
    ...
    logger:info("All other objects undistrubed"),
    read_from_cluster(NodeA, 101, 900, ?COMMMON_VAL_INIT, 0),
    read_from_cluster(NodeA, 901, 1000, ?COMMMON_VAL_INIT, 100),
    read_from_cluster(NodeB, 101, 900, ?COMMMON_VAL_INIT, 0),
    read_from_cluster(NodeB, 901, 1000, ?COMMMON_VAL_INIT, 100),
    ...
    logger:info("Replace sibling on Node A"),
    ...
    logger:info("Full sync from C to A will find deltas but no repairs"),
    {clock_compare, 100}
    ...
    logger:info("Full sync from B to C should re-align all"),
    {clock_compare, 100}
        = fullsync_check({NodeB, IPB, PortB, ?B_NVAL},
                            {NodeC, IPC, PortC, ?C_NVAL},
                            cluster_c),
