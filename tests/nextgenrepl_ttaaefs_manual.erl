...
test_repl_between_clusters(ClusterA, ClusterB, ClusterC,
    ...
    logger:info("Test 100 key difference and resolve"),
    ...
    {clock_compare, N1}
        = FullSyncFun({NodeA, IPA, PortA, ?A_NVAL},
                        {NodeB, IPB, PortB, ?B_NVAL}),
    logger:info("First comparison found ~w differences", [N1]),
    ?assertEqual(true, N1 > 100),
    ?assertEqual(true, N1 < 1000),

    logger:info("Further eight loops should complete repair"),
    LoopRepairFun =
        fun(SrcInfo, SnkInfo) ->
            fun(_I) ->
    ...
    {root_compare, 0} =
        FullSyncFun({NodeB, IPB, PortB, ?B_NVAL},
                    {NodeA, IPA, PortA, ?A_NVAL}),
    logger:info("NodeA and NodeB has been re-sync'd"),
    read_from_cluster(NodeB, 1, 1100, 0),

    logger:info("Repairing in the wrong direction doesn't repair"),
    {clock_compare, 128} =
        FullSyncFun({NodeC, IPC, PortC, ?C_NVAL},
                    {NodeB, IPB, PortB, ?B_NVAL}),
    read_from_cluster(NodeC, 101, 1100, 1000),

    logger:info("Complete repair from different clusters"),
    lists:foreach(LoopRepairFun({NodeA, IPA, PortA, ?A_NVAL},
                                {NodeC, IPC, PortC, ?C_NVAL}),
                    lists:seq(1, 5)),
    logger:info("... reading after repairs from node A"),
    read_from_cluster(NodeC, 1, 1100, undefined),
    lists:foreach(LoopRepairFun({NodeB, IPB, PortB, ?B_NVAL},
                                {NodeC, IPC, PortC, ?C_NVAL}),
                    lists:seq(1, 5)),
    logger:info("... reading after repairs from node B"),
    read_from_cluster(NodeC, 1, 1100, undefined),
    ...
    logger:info("Confirm that replicating back doesn't remove tombstones ..."),
    logger:info("... But it will see differences"),
    {clock_compare, 100} =
        FullSyncFun({NodeC, IPC, PortC, ?C_NVAL},
                        {NodeA, IPA, PortA, ?A_NVAL}),
    read_from_cluster(NodeA, 901, 1000, 100),
    logger:info("Confirm that replicating forward does migrate tombstones"),
    {clock_compare, 100} =
        FullSyncFun({NodeA, IPA, PortA, ?A_NVAL},
                        {NodeB, IPB, PortB, ?B_NVAL}),
    ...
    case PartialSyncFun of
        none ->
            pass;
        _ ->
            logger:info("Compare the bucket - dynamic AAE not based on cached trees"),
            Now = os:timestamp(),
            timer:sleep(1000),
            {tree_compare, 0} =
            ...
            logger:info("Test further 1000 key differences"),
            ...
            {clock_compare, N2}
                = PartialSyncFun({NodeA, IPA, PortA, ?A_NVAL},
                                    ...
                                    hour_check,
                                    os:timestamp()),
            logger:info("First comparison found ~w differences", [N2]),
            ...
            {tree_compare, 0} =
                PartialSyncFun({NodeA, IPA, PortA, ?A_NVAL},
                                ...
                                hour_check,
                                os:timestamp()),
            logger:info("Differences A -> B resolved"),

            logger:info("Wind now back to before changes - and don't see differences"),
            ...
            {tree_compare, 0} =
                PartialSyncFun({NodeA, IPA, PortA, ?B_NVAL},
                                ...
                                hour_check,
                                Now),
            logger:info("Look from now - and see the differences"),
            {clock_compare, N3}
                = PartialSyncFun({NodeA, IPA, PortA, ?A_NVAL},
                                    {NodeC, IPC, PortC, ?C_NVAL},
                                    hour_check,
            ...
            logger:info("Five more syncs should fix it"),
            lists:foreach(LoopPartialRepairFun({NodeA, IPA, PortA, ?A_NVAL},
                                                {NodeC, IPC, PortC, ?C_NVAL},
                                                os:timestamp()),
                    lists:seq(1, 5)),

            logger:info("Loop around syncing to confirm all OK"),
            {root_compare, 0} =
                FullSyncFun({NodeA, IPA, PortA, ?A_NVAL},
                                {NodeB, IPB, PortB, ?B_NVAL}),
            {root_compare, 0} =
...
test_externalrepl_between_clusters(ClusterA, ClusterB) ->
    logger:info("spoof reconciliation with an external cluster"),
...
fullsync_check({SrcNode, SrcIP, SrcPort, SrcNVal},
    ...
    logger:info("Drained queue and pushed ~w objects", [N]),
    AAEResult.
