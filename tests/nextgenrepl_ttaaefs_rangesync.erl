...
test_repl_between_clusters(ClusterA, ClusterB, ClusterC) ->
    ...
    logger:info("Root compare has not set range check"),
    lists:foreach(fun(N) -> ?assertMatch(none, get_range(N)) end,
                    [NodeA, NodeB, NodeC]),
    
    logger:info("Range check should also root compare"),
    {root_compare, 0}
    ...
    logger:info("Range check now resolves A -> B"),
    {clock_compare, 68}
        = RangeCheckFun({NodeA, IPA, PortA, ?A_NVAL},
                            {NodeB, IPB, PortB, ?B_NVAL}),
    ?assertMatch(?TEST_BUCKET, element(1, get_range(NodeA))),
    ...
    logger:info("On startup - range_check should check since startup"),
    logger:info("Everything should be fixed due to range boost"),
    {clock_compare, 100}
        = RangeCheckFun({NodeB, IPB, PortB, ?B_NVAL},
                        {NodeC, IPC, PortC, ?C_NVAL}),
    {tree_compare, 0}
    ...
    logger:info("Cluster B should be N=3 and nodes=2, so can cope with node out of coverage"),
    ...
    logger:info("something bad now happens with NodeA out of coverage"),
    {error, 0}
        = partialsync_check({NodeA, IPA, PortA, ?A_NVAL},
                            {NodeC, IPC, PortC, ?C_NVAL},
                            all_check,
