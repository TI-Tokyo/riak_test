...
confirm() ->
    ...
    logger:info("Got preflist"),
    logger:info("Preflist ~p~n", [PL]),
    [FirstNode | OtherPrimaries] = [Node || {{_Idx, Node}, Type} <- PL,
                                    Type == primary],
    logger:info("Other Primaries ~p~n", [OtherPrimaries]),

    [rt:stop_and_wait(N) || N <- OtherPrimaries],
    logger:info("Killed 2 primaries"),
    ...
    logger:info("Change bucket properties to pw:2 node_confirms:0"),
    ...
    logger:info("Attempting to write key"),
    ...
    logger:info("Change bucket properties to pw:0 node_confirms:2"),
    ...
    logger:info("Read back key with node_confirms = 2"),
    ...
    logger:info("Change bucket properties to pw:0 node_confirms:4"),
    ...
    logger:info("Got preflist"),
    logger:info("Preflist ~p~n", [PL2]),

    logger:info("Change bucket properties to pw:0 node_confirms:3"),
    ...
    logger:info("Left with ~w nodes ~w and ~w primary vnodes",
    ...
    logger:info("Setting pw and node_confirms to reflect current status"),
    rpc:call(FirstNode,
    ...
    logger:info("Read back key"),
    logger:info("Although previous put failed, it will have been co-ordinated"),
    logger:info("We can expect to see siblings - both PUTs should be present"),
    logger:info("Success occurs as enough nodes and primaries are up"),
    {ok, ObjS2} = rhc:get(Client, ?BUCKET, ?KEY),
    ?assertMatch([<<"12345">>, <<"12346">>],
                    lists:usort(riakc_obj:get_values(ObjS2))),
    logger:info("Incrementing node_confirms to cause failure"),
    rpc:call(FirstNode,
                riak_core_bucket, set_bucket,
                [?BUCKET,
    ...
    logger:info("Error ~p now returned", [Error]),
    ExpectedError =
        list_to_binary(
    ...
    logger:info("Test when setting notfound_ok and with one node down"),
    logger:info("Should be able to confirm with node_confirms=3 if notfound_ok"),
    logger:info("But if not notfound_ok, should not be able to read initially"),
    logger:info("And this state will be corrected by read repair"),
    {C0, _PbC0, FailedNode0} =
        setup_notfound_test(Node1, <<"notfound_key0">>, false),

    logger:info("Read should fail first time if notfound_ok is false"),
    {error, {ok, "500", _AltHeaders, AltError}} =
        rhc:get(C0, ?BUCKET, <<"notfound_key0">>),
    logger:info("Error ~p now returned", [AltError]),
    AltExpectedError =
        list_to_binary(
            io_lib:format("Error:\n{insufficient_nodes,~w,need,~w}\n",
                            [2, 3])),
    ...
    logger:info("Because of read repair - read will soon succeed"),
    ...
    logger:info("Read should succeed first time if notfound_ok is true"),
    ...
    logger:info("Repeat test, this time overriding node_confirms using option"),
    {C2, _PbC2, FailedNode2} =
        setup_notfound_test(Node1, <<"notfound_key2">>, false),
    
    logger:info("Read should succeed first time if override node_confirms"),
    {ok, ObjS6} =
    ...
    logger:info("Repeat test, this time overriding in PB Client"),
...
setup_notfound_test(Node1, Key, NotFoundOK) ->
    logger:info("Testing with notfound_ok ~w", [NotFoundOK]),
    ...
    logger:info("Write new object in re-formed cluster"),
    rt:httpc_write(HttpC, ?BUCKET, Key, <<"ABCDE">>),

    logger:info("Stop a single primary"),
    rt:stop_and_wait(PriC),
    logger:info("Target_n_val should be 4"),
    logger:info("So new preflist should be on 3 nodes"),
    AltPL2 = rt:get_preflist(PriA, ?BUCKET, Key),
    ?assertMatch(3, length(
                        lists:usort(
                            lists:map(fun({{_Idx, AltN}, _Status}) -> AltN end,
                                        AltPL2)))),
    logger:info("Setting pw to 2 and node_confirms to 3"),
    rpc:call(PriA,
                riak_core_bucket, set_bucket,
                [?BUCKET,
                    [{'pw', 2},
