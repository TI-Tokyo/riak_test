...
confirm() ->
    ...
    logger:info("Good ciphers: ~p", [GoodCiphers]),
    [AES256, AES128, _ECDSA] = 
        ParsedCiphers = 
            lists:map(fun(PC) -> cipher_format(PC) end, GoodCiphers),
    
    logger:info("Parsed Ciphers ~w", [ParsedCiphers]),

    logger:info("Check that the server's preference for ECDHE-RSA-AES256-SHA384"
               " is honored"),
    ...
    logger:info("With cipher order - ~w", [AES256R]),
    ?assertEqual(AES256T, {element(1, AES256R),
                            element(2, AES256R),
                            element(3, AES256R)}),
    logger:info("Ignoring reversal of cipher order!!"),
    {ok, {'tlsv1.2', AES256R}} =
        pb_connection_info(Port,
                            [{credentials, "user", "password"},
                            {cacertfile,
    ...
    logger:info("Do we assume that cipher order is not honoured?"),
    
    SingleCipherProps =
        [{credentials, "user", "password"},
            {cacertfile, filename:join([CertDir, "rootCA/cert.pem"])},
            {ssl_opts, [{ciphers, [AES128]}]}],
    logger:info("Setting weak cipher now throws insufficient security"),
    insufficient_check(Port, SingleCipherProps),

    logger:info("check that connections trying to use tls 1.1 fail"),
    ...
    logger:info("check that connections trying to use tls 1.0 fail"),
    {error,{tcp,{tls_alert,ProtocolVersionError}}} =
        pb_connection_info(Port,
                            [{credentials, "user", "password"},
                                {cacertfile,
    ...
    logger:info("check that connections trying to use ssl 3.0 fail"),
    {error,{tcp,{tls_alert,ProtocolVersionError}}} =
        pb_connection_info(Port,
                            [{credentials, "user", "password"},
                                {cacertfile,
    ...
    logger:info("Enable ssl 3.0, tls 1.0 and tls 1.1 and disable tls 1.2"),
    rpc:call(Node, application, set_env, [riak_api, tls_protocols,
                                            [sslv3, tlsv1, 'tlsv1.1']]),

    logger:info("check that connections trying to use tls 1.2 fail"),
    ...
    logger:info("Re-enabling old protocols will work in OTP 22"),
    check_with_reenabled_protools(Port, CertDir),

    logger:info("Reset tls protocols back to the default"),
    rpc:call(Node, application, set_env, [riak_api, tls_protocols,
                                          ['tlsv1.2']]),

    logger:info("checking CRLs are checked for client certificates by"
              " default"),
    ...
    logger:info("Disable CRL checking"),
    rpc:call(Node, application, set_env, [riak_api, check_crl,
                                          false]),

    logger:info("Checking revoked certificates are allowed"),
...
check_reasons(ProtocolVersionError) ->
    logger:info("Unexpected error ~s", [ProtocolVersionError]),
    error.

check_with_reenabled_protools(Port, CertDir) ->
    logger:info("Check tls 1.1 succeeds - OK as long as cipher good?"),
    logger:info("Note that only 3-tuple returned for 1.1 - and sha not sha384"),
    ...
    logger:info("check tls 1.0 succeeds - OK as long as cipher is good?"),
...
-else.
    ...
    -ifdef(deprecated_21).
        ...
        check_with_reenabled_protools(Port, CertDir) ->
            logger:info("Cannot re-enable old protocols before OTP 22"),
