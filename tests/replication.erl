...
replication([AFirst|_] = ANodes, [BFirst|_] = BNodes, Connected) ->
    ...
    logger:info("Writing 100 more keys to ~p", [LeaderA]),
    ?assertEqual([], do_write(LeaderA, 101, 200, TestBucket, 2)),

    rt:log_to_nodes(AllNodes, "Verify data received on B"),
    %% verify data is replicated to B
    logger:info("Reading 100 keys written to ~p from ~p", [LeaderA, BFirst]),
    ...
    logger:info("Writing 100 more keys to ~p now that the old leader is down",
        [ASecond]),
    ...
    logger:info("Restarting down node ~p", [LeaderA]),
    rt:start(LeaderA),
    ...
    case nodes_all_have_version(ANodes, "1.2.2") of
        true ->
            ...
            logger:info("Starting Joe's Repl Test"),
            ...
            logger:info("Leader: ~p", [rpc:call(ASecond, riak_repl_leader, leader_node, [])]),
            logger:info("LeaderA: ~p", [LeaderA]),
            logger:info("LeaderA2: ~p", [LeaderA2]),

            ?assertEqual(ok, wait_until_connection(LeaderA)),

            logger:info("Simulation partition to force leader re-election"),
            ...
            logger:info("Leader: ~p", [LeaderA3]),
            logger:info("Writing 2 more keys to ~p", [LeaderA3]),
            ?assertEqual([], do_write(LeaderA3, 1301, 1302, TestBucket, 2)),

            %% verify data is replicated to B
            logger:info("Reading 2 keys written to ~p from ~p", [LeaderA3, BSecond]),
            ?assertEqual(0, wait_for_reads(BSecond, 1301, 1302, TestBucket, 2)),

            logger:info("Finished Joe's Section"),

            logger:info("Nodes restarted");
        _ ->
            logger:info("Skipping Joe's Repl Test")
    end,

    logger:info("Restarting down node ~p", [LeaderB]),
    rt:start(LeaderB),
        ...
        true ->
            ...
            case nodes_all_have_version(ANodes, "1.2.0") of
                true ->
                    ...
                    logger:info("write 100 keys to a realtime only bucket"),
            ...
            case nodes_all_have_version(ANodes, "1.2.0") of
                true ->
                    ...
                    logger:info("Check the realtime only bucket that was written to offline "
                        "isn't replicated"),
            ...
            case nodes_all_have_version(ANodes, "1.2.0") of
                true ->
                    ...
                    logger:info("Write 100 more keys into realtime only bucket"),
...
del_listeners(Node) ->
    ...
    lists:foreach(fun(Listener={IP, Port, N}) ->
                logger:info("deleting listener ~p on ~p", [Listener, Node]),
...
start_and_wait_until_fullsync_complete(Node, Retries) ->
    ...
    case rt:wait_until(make_fullsync_wait_fun(Node, Count), 100, 1000) of
        ...
        _  when Retries > 0 ->
            ...
            logger:warning("Node failed to fullsync, retrying"),
...
wait_until_is_leader(Node) ->
    logger:info("wait_until_is_leader(~p)", [Node]),
...
is_leader(Node) ->
    case rpc:call(Node, riak_repl_leader, leader_node, []) of
        {badrpc, _} ->
            ...
            false;
        Leader ->
            logger:info("Checking: ~p =:= ~p", [Leader, Node]),
            Leader =:= Node
    end.


wait_until_is_not_leader(Node) ->
    logger:info("wait_until_is_not_leader(~p)", [Node]),
...
do_write(Node, Start, End, Bucket, W) ->
    case rt:systest_write(Node, Start, End, Bucket, W) of
        [] ->
            [];
        Errors ->
            logger:warning("~p errors while writing: ~p",
