...
confirm() ->
    ...
    logger:info("Building Clusters A and B"),
...
replication(ANodes, BNodes, Connected) ->
    ...
    logger:info("Connection Status: ~p", [Connected]),

    logger:info("Real Time Full Sync Replication test"),
    real_time_replication_test(ANodes, BNodes, Connected),
 
    logger:info("Disconnected cluster Full Sync test"),
    disconnected_cluster_fsync_test(ANodes, BNodes),

    logger:info("Failover tests"),
    master_failover_test(ANodes, BNodes),

    logger:info("Network Partition test"),
    network_partition_test(ANodes, BNodes),

    logger:info("Bucket Sync tests"),
    bucket_sync_test(ANodes, BNodes), 

    logger:info("Offline queueing tests"),
    offline_queueing_tests(ANodes, BNodes),

    logger:info("Protocol Buffer writes during shutdown test"),
    pb_write_during_shutdown(ANodes, BNodes),

    logger:info("HTTP writes during shutdown test"),
...
real_time_replication_test([AFirst|_] = ANodes, [BFirst|_] = BNodes, Connected) ->
    ...
    case Connected of
        false ->
            ...
            logger:info("Writing 100 keys to ~p", [AFirst]),
            ...
            logger:info("connect cluster A:~p to B on port ~p", [LeaderA, BFirstPort]),
        ...
        _ ->
            logger:info("Clusters should already be connected"),
            logger:info("Waiting for leader to converge on cluster A"),
            ?assertEqual(ok, repl_util:wait_until_leader_converge(ANodes)),
            logger:info("Waiting for leader to converge on cluster B"),
            ...
            logger:info("Leader on cluster A is ~p", [LeaderA]),
            logger:info("BFirst on cluster B is ~p", [BFirst]),
            {ok, {_IP, BFirstPort}} = rpc:call(BFirst, application, get_env, [riak_core, cluster_mgr]),
            logger:info("B is ~p with port ~p", [BFirst, BFirstPort]) 
    end,

    log_to_nodes(ANodes++BNodes, "Write data to Cluster A, verify replication to Cluster B via realtime"),
    logger:info("Writing 100 keys to Cluster A-LeaderNode: ~p", [LeaderA]), % This export from Case isn't my favorite.
    ?assertEqual([], repl_util:do_write(LeaderA, 101, 200, TestBucket, 2)),

    logger:info("Reading 100 keys written to Cluster A-LeaderNode: ~p from Cluster B-Node: ~p", [LeaderA, BFirst]),
    ...
    case Connected of
        false ->
            ...
            logger:info("Check keys written before repl was connected are present"),
    ...
    logger:info("Deleting 100 keys on Cluster A"),
...
disconnected_cluster_fsync_test([AFirst|_] = ANodes, [BFirst|_] = BNodes) ->
    ...
    logger:info("Disconnect the 2 clusters"),
    ...
    logger:info("Write 2000 keys"),
    ?assertEqual([], repl_util:do_write(LeaderA, 50000, 52000, TestBucket, 2)),

    logger:info("Reconnect the 2 clusters"),
    repl_util:connect_cluster(LeaderA, "127.0.0.1", BFirstPort),
    ?assertEqual(ok, repl_util:wait_for_connection(LeaderA, "B")),
    rt:wait_until_ring_converged(ANodes),
    repl_util:enable_realtime(LeaderA, "B"),
...
master_failover_test([AFirst|_] = ANodes, [BFirst|_] = BNodes) ->
    ...
    logger:info("Testing master failover: stopping ~p", [LeaderA]),
    rt:stop(LeaderA),
    ...
    logger:info("Testing client failover: stopping ~p", [LeaderB]),
    rt:stop(LeaderB),
    ...
    logger:info("New leader is ~p", [LeaderB2]),
    ?assertEqual(ok, repl_util:wait_until_connection(LeaderA2)),
    ...
    %% Verify data is replicated to Cluster B
    logger:info("Reading 101 keys written to ~p from ~p", [ASecond, BSecond]),
    ...
    logger:info("Re-running fullsync with ~p and ~p down", [LeaderA, LeaderB]),
    ...
    logger:info("Nodes restarted"),
    logger:info("Restarting down nodes ~p, ~p", [LeaderA, LeaderB]),
    rt:start(LeaderA),
    rt:start(LeaderB),
...
network_partition_test([AFirst|_] = ANodes, [BFirst|_] = BNodes) ->
    ...
    %%logger:info("Leader: ~p", [rpc:call(ASecond, riak_core_cluster_mgr, get_leader, [])]),
    %%logger:info("LeaderA: ~p", [LeaderA]),
    %%logger:info("LeaderA2: ~p", [LeaderA2]),
    ...
    logger:info("Interim leader: ~p", [InterimLeader]),
    ...
    logger:info("Leader: ~p", [rpc:call(ASecond, riak_core_cluster_mgr, get_leader, [])]),
    logger:info("Writing 2 more keys to ~p", [LeaderA]),
    ?assertEqual([], repl_util:do_write(LeaderA, 1301, 1302, TestBucket, 2)),

    %% Verify data is replicated to Cluster "B"
    logger:info("Reading 2 keys written to ~p from ~p", [LeaderA, BFirst]),
...
bucket_sync_test([AFirst|_] = ANodes, [BFirst|_] = BNodes) ->
    ...
    logger:info("Write 100 keys to a real time only bucket"),
    ?assertEqual([], repl_util:do_write(AFirst, 1, 100, RealtimeOnly, 2)),

    logger:info("Reconnect Clusters A and B"),
    repl_util:connect_cluster(LeaderA, "127.0.0.1", BFirstPort),
    ?assertEqual(ok, repl_util:wait_for_connection(LeaderA, "B")),
    rt:wait_until_ring_converged(ANodes),
    repl_util:enable_realtime(LeaderA, "B"),
    ...
    logger:info("write 100 keys to a {repl, false} bucket"),
    ?assertEqual([], repl_util:do_write(AFirst, 1, 100, NoRepl, 2)),

    logger:info("write 100 keys to a fullsync only bucket"),
    ?assertEqual([], repl_util:do_write(AFirst, 1, 100, FullsyncOnly, 2)),

    logger:info("Check the fullsync only bucket didn't replicate the writes"),
    Res6 = rt:systest_read(BFirst, 1, 100, FullsyncOnly, 2),
    ?assertEqual(100, length(Res6)),

    logger:info("Check the realtime only bucket that was written to offline isn't replicated"),
    Res7 = rt:systest_read(BFirst, 1, 100, RealtimeOnly, 2),
    ?assertEqual(100, length(Res7)),

    logger:info("Check the {repl, false} bucket didn't replicate"),
    ...
    logger:info("Check fullsync only bucket is now replicated"),
    ?assertEqual(0, repl_util:wait_for_reads(BFirst, 1, 100, FullsyncOnly, 2)),

    logger:info("Check realtime only bucket didn't replicate"),
    Res10 = rt:systest_read(BFirst, 1, 100, RealtimeOnly, 2),
    ?assertEqual(100, length(Res10)),

    logger:info("Write 100 more keys into realtime only bucket on ~p", [AFirst]),
    ?assertEqual([], repl_util:do_write(AFirst, 101, 200, RealtimeOnly, 2)),

    logger:info("Check the realtime keys replicated"),
    ?assertEqual(0, repl_util:wait_for_reads(BFirst, 101, 200, RealtimeOnly, 2)),

    logger:info("Check the older keys in the realtime bucket did not replicate"),
    Res12 = rt:systest_read(BFirst, 1, 100, RealtimeOnly, 2),
    ?assertEqual(100, length(Res12)),

    logger:info("Check {repl, false} bucket didn't replicate"),
...
offline_queueing_tests([AFirst|_] = ANodes, [BFirst|_] = BNodes) ->
    ...
    logger:info("Testing offline realtime queueing"),

    LeaderA = rpc:call(AFirst, riak_core_cluster_mgr, get_leader, []),

    logger:info("Stopping realtime, queue will build"),
    repl_util:stop_realtime(LeaderA, "B"),
    rt:wait_until_ring_converged(ANodes),

    logger:info("Writing 100 keys"),
    ?assertEqual([], repl_util:do_write(LeaderA, 800, 900, TestBucket, 2)),

    logger:info("Starting realtime"),
    repl_util:start_realtime(LeaderA, "B"),
    rt:wait_until_ring_converged(ANodes),
    ?assertEqual(ok, repl_util:wait_for_connection(LeaderA, "B")),

    logger:info("Reading keys written while repl was stopped"),
    ...
    logger:info("Testing realtime migration on node shutdown"),
    ...
    logger:info("Verifying 100 keys are now available on ~p", [BFirst]),
...
pb_write_during_shutdown([AFirst|_] = ANodes, [BFirst|_] = BNodes) ->
    ...
    logger:info("Connecting to pb socket ~p:~p on ~p", [IP, Port, Target]),
    PBSock = rt:pbc(Target),

    %% do the stop in the background while we're writing keys
    spawn(fun() ->
              timer:sleep(500),
              logger:info("Stopping node ~p again", [Target]),
              rt:stop(Target),
              logger:info("Node stopped")
          end),

    logger:info("Writing 10,000 keys"),
    WriteErrors =
        try
            pb_write(PBSock, 1000, 11000, TestBucket, 2)
        catch
      _:_ ->
          logger:info("Shutdown timeout caught"),
          []
      end,
    logger:info("Received ~p write failures", [length(WriteErrors)]),
    timer:sleep(3000),
    logger:info("Checking number of read failures on secondary cluster"),
    ReadErrors = rt:systest_read(BFirst, 1000, 11000, TestBucket, 2),
    logger:info("Received ~p read failures", [length(ReadErrors)]),

    %% Ensure node is down before we try to start it up again.
    logger:info("pb_write_during_shutdown: Ensure node ~p is down before restart", [Target]),
    ...
    logger:info("Received ~p read failures on ~p", [length(ReadErrors2), Target]),
    case length(WriteErrors) >= length(ReadErrors) of
        true ->
            ok;
        false ->
            logger:error("Received more read errors on ~p: ~p than write errors on ~p: ~p",
    ...
    logger:info("Failed keys ~p", [FailedKeys]),
    ?assert(false)
    end.
...
http_write_during_shutdown([AFirst|_] = ANodes, [BFirst|_] = BNodes) ->
    ...
    logger:info("Connecting to http socket ~p:~p on ~p", [IP, Port, Target]),
    C = rt:httpc(Target),
    ...
    logger:info("got ~p write failures to ~p", [length(WriteErrors), Target]),
    timer:sleep(3000),
    logger:info("Checking number of read failures on secondary cluster node, ~p", [BFirst]),
    [{_, {IP, Port2}},_] = rt:connection_info(BFirst),
    C2 = rhc:create("127.0.0.1", Port2, "riak", []),
    ReadErrors = http_read(C2, 12000, 22000, TestBucket, 2),
    logger:info("Received ~p read failures from ~p", [length(ReadErrors), BFirst]),

    %% Ensure node is down before we try to start it up again.
    logger:info("HTTP: write_during_shutdown: Ensure node ~p is down before restart", [Target]),
    ?assertEqual(ok, rt:wait_until_unpingable(Target)),
