...
confirm() ->
    ...
    logger:info("Waiting until all nodes clean"),
    ...
    logger:info("Writing 2000 more keys to ~p", [LeaderA]),
    ?assertEqual([], repl_util:do_write(LeaderA, 101, 2000, TestBucket, 2)),

    %% verify data is replicated to B
    logger:info("Reading 2000 keys written to ~p from ~p", [LeaderA, BFirst]),
    ...
    logger:info("Manually setting rt_dirty state"),
    ...
    logger:info("Waiting until dirty"),
    wait_until_coord_has_dirty(LeaderA),

    logger:info("Starting fullsync"),
    repl_util:start_and_wait_until_fullsync_complete(LeaderA),
    logger:info("Wait for all nodes to show up clean"),
    wait_until_all_nodes_clean(LeaderA),

    %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    rt:log_to_nodes(AllNodes, "Multiple node test"),
    logger:info("Multiple node test"),
    %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    ...
    logger:info("Result = ~p", [ResultA]),
    logger:info("Result = ~p", [ResultB]),
    ...
    logger:info("Multiple node test, one failed during fullsync"),
    %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    %% test multiple nodes dirty
    ...
    spawn(fun() ->
                timer:sleep(1000),
                logger:info("Marking node as dirty during a fullsync"),
                ResultC = rpc:call(DirtyD, riak_repl_stats, rt_source_errors, []),
                logger:info("Result = ~p", [ResultC])
           end),
    repl_util:start_and_wait_until_fullsync_complete(LeaderA),

    logger:info("Checking to see if C is still clean"),
    wait_until_node_clean(DirtyC),
    logger:info("Checking to see if D is still dirty"),
...
get_dirty_stat(Node) ->
    ...
    %logger:info("RT_DIRTY = ~p", [proplists:get_value(rt_dirty, Stats, -1)]),
...
get_rt_errors(Node) ->
    ...
    logger:info("Source errors = ~p, sink errors = ~p", [SourceErrors, SinkErrors]),
    {SourceErrors, SinkErrors}.

wait_until_coord_has_dirty(Node) ->
    Res = rt:wait_until(Node,
                        fun(_) ->
                    logger:info("Checking dirty for node ~p", [Node]),
...
%                        fun(_) ->
%                    logger:info("Checking for any dirty nodes"),
...
%    Res = rt:wait_until(SourceLeader,
%                        fun(_) ->
%                    logger:info("Writing data while checking for any dirty nodes"),
...
wait_until_all_nodes_clean(Leader) ->
    Res = rt:wait_until(Leader,
                        fun(L) ->
                    logger:info("Checking for all nodes clean"),
