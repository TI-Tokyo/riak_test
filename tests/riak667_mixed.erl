...
confirm() ->
    ...
    logger:info("mdc_crdt_epoch? ~p", [rpc:multicall(Nodes, application, get_env, [riak_kv, mdc_crdt_epoch])]),
    ...
    logger:notice("running mixed 2.0.2 and 2.0.4"),
    ...
    logger:notice("Can't read a 2.0.2 map from 2.0.4 node"),
    ...
    logger:notice("Created a 2.0.4 map"),
    ...
    logger:notice("Can't read 2.0.4 map from 2.0.2 node"),
    ...
    logger:notice("running mixed 2.0.2 and ~s", [CurrentVer]),
    ...
    logger:notice("2.0.2 map ~p", [K1O]),
    logger:notice("2.0.4 map ~p", [K2O]),
    ...
    logger:notice("Updated 2.0.2 map on ~s", [CurrentVer]),

    %% read 2.0.2 map from 2.0.2 node ?KEY Pid
    {ok, K1OR} = riakc_pb_socket:fetch_type(N1C2, ?BUCKET, ?KEY),
    logger:notice("Read 2.0.2 map from 2.0.2 node: ~p", [K1OR]),
    ...
    logger:notice("Updated 2.0.2 map on 2.0.2 node"),

    %% read it from 2.0.5 node ?KEY Pid3
    {ok, K1OC} = riakc_pb_socket:fetch_type(N2C3, ?BUCKET, ?KEY),
    logger:notice("Read 2.0.2 map from ~s node: ~p", [CurrentVer, K1OC]),
    ...
    logger:notice("Updated 2.0.4 map on ~s node", [CurrentVer]),
    %% upgrade 2.0.2 node

    riakc_pb_socket:stop(N1C2),
    upgrade(Node1, current),
    logger:notice("Upgraded 2.0.2 node to ~s", [CurrentVer]),
    ...
    logger:notice("Maps fetched from both nodes are same K1:~p K2:~p", [K1N1, K2N1]),
    ...
    logger:notice("Maps fetched from both nodes are same K1:~p K2:~p", [Up1N1, Up2N1]),
...
upgrade(Node, NewVsn) ->
    logger:notice("Upgrading ~p to ~p", [Node, NewVsn]),
...
map_contents_are_lists(RObj) ->
    ...
    logger:info("Top-level map: ~p || ~p", [Entries, Deferred]),
...
map_from_binary(<<69:8, 2:8, TypeLen:32/integer, Type:TypeLen/binary, 77:8, MapVer:8,
                  CRDTBin/binary>>) ->
    logger:notice("Deserialized Map: ~s v~p", [Type, MapVer]),
