...
make_noise_if_results_not_empty(RealResults, {ok, {_, RealResults}},
                                Port, Description, Context) ->
    logger:info("Stopping test, we got results when we didn't want any"),
    ContextUnwrapped = binary_to_term(Context),
    logger:info("Retrieved real data from ~p : ~p~n", [Port, Description]),
    logger:info("Context: ~p~n", [ContextUnwrapped]),
    logger:info("Asked from wrong port: ~p~n", [alternate_port(Port)]),
    logger:info("Got same results from both nodes"),
...
make_noise_if_results_not_empty(_RealResults, {ok, {_, _BogusResults}},
    ...
    logger:info("Context: ~p~n", [ContextUnwrapped]),
    logger:info("Asked from wrong port: ~p~n", [alternate_port(Port)]),
    logger:info("Got different (non-empty) results from second node"),
