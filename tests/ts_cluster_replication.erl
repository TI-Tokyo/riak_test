...
confirm() ->
    rt_config:set(num_nodes, NumNodes = 6),
...
test_ddl_comparison(ANodes, BNodes) ->
    logger:info("Testing TS realtime fails with incompatible DDLs"),
...
create_normal_type(Nodes, BucketType) ->
    TypeProps = [{n_val, 3}],
    logger:info("Create bucket type ~p, wait for propagation", [BucketType]),
...
delete_record(Node, Table, Time) ->
    ...
    logger:info("Deleting ~p from ~ts~n", [KeyAsList, Table]),
...
replication(ANodes, BNodes, Table, NormalType) ->
    ...
    logger:info("Adding intercept to RTQ"),
    ...
    logger:info("Writing 100 KV values to ~p (~p)", [hd(ANodes), KVBucket]),
    ?assertEqual([], repl_util:do_write(hd(ANodes), 1, 100, KVBucket, 2)),

    logger:info("Writing 100 KV values to ~p (~p)", [hd(ANodes), KVBucketInTS]),
    ?assertEqual([], repl_util:do_write(hd(ANodes), 1, 100, KVBucketInTS, 2)),

    logger:info("Writing 100 TS records to ~p (~p)", [hd(ANodes), Table]),
    ...
    logger:info("Checking to see whether anything was (wrongly) pushed to the RTQ"),
    WasPushed = rpc:call(hd(ANodes), application, get_env, [riak_repl, '_test_rtq_pushed']),
    ?assertEqual(undefined, WasPushed),

    logger:info("Testing a non-w1c bucket type (realtime)"),
    ...
    logger:info("Testing the timeseries bucket type, non-ts-managed bucket (realtime)"),
    no_w1c_real_time_replication_test(ANodes, BNodes, LeaderA, PortB, KVBucketInTS),

    logger:info("Testing timeseries data (realtime)"),
    ts_real_time_replication_test(ANodes, BNodes, LeaderA, PortB, Table),

    logger:info("Testing a change of bucket properties to disable realtime"),
    no_ts_real_time_replication_test(ANodes, BNodes, LeaderA, PortB, Table),

    logger:info("Testing all buckets with fullsync"),
...
full_sync_replication_test([AFirst|_]=ANodes, [BFirst|_]=BNodes, LeaderA, PortB, KVBucket, KVBucketInTS, Table) ->
    ...
    logger:info("Verifying first 100 keys not present on 2nd cluster (non-w1c)"),
    ?assertEqual(0, kv_num_objects_present(BNode, 1, 100, KVBucket)),
    logger:info("Verifying first 100 keys not present on 2nd cluster (ts bucket type, non-ts bucket)"),
    ?assertEqual(0, kv_num_objects_present(BNode, 1, 100, KVBucketInTS)),
    logger:info("Verifying first 100 TS keys not present on 2nd cluster"),
    ?assertEqual(0, ts_num_records_present(BNode, 1, 100, Table)),

    logger:info("Starting and waiting for fullsync"),
    ...
    logger:info("Verifying first 100 keys present on 2nd cluster (non-w1c)"),
    ?assertEqual(100, kv_num_objects_present(BNode, 1, 100, KVBucket)),
    logger:info("Verifying first 100 keys present on 2nd cluster (ts bucket type, non-ts bucket)"),
    ?assertEqual(100, kv_num_objects_present(BNode, 1, 100, KVBucketInTS)),
    logger:info("Verifying first 100 TS keys present on 2nd cluster"),
    ?assertEqual(100, ts_num_records_present(BNode, 1, 100, Table)),

    logger:info("Deleting a record on Cluster A"),
    delete_record(AFirst, Table, 23),
    timer:sleep(500),
    logger:info("Verifying record is no longer on Cluster A"),
    ?assertEqual(0, ts_num_records_present(AFirst, 23, 23, Table)),
    repl_util:start_and_wait_until_fullsync_complete(ANodes, "B"),
    logger:info("Verifying record is no longer on Cluster B"),
...
real_time_replication_test(ANodes, [BFirst|_] = BNodes, LeaderA, PortB, Bucket) ->
    ...
    connect_clusters(ANodes, BNodes, LeaderA, PortB),
    logger:info("Starting real-time MDC"),
    ...
    logger:info("Writing 100 keys to Cluster A-LeaderNode: ~p", [LeaderA]),
    ?assertEqual([], repl_util:do_write(LeaderA, 101, 200, Bucket, 2)),

    logger:info("Reading 100 keys written to Cluster A-LeaderNode: ~p from Cluster B-Node: ~p", [LeaderA, PortB]),
...
ts_real_time_replication_test([AFirst|_] = ANodes, [BFirst|_] = BNodes, LeaderA, PortB, Table) ->
    ...
    logger:info("Verifying none of the records on A are on B"),
    ...
    logger:info("Writing a single key to Cluster A-LeaderNode: ~p", [LeaderA]),
    ?assertEqual(ok, put_records(AFirst, Table, 201, 201)),

    logger:info("Reading 101 keys written to Cluster A-LeaderNode: ~p from Cluster B-Node: ~p", [LeaderA, BFirst]),
...
no_w1c_real_time_replication_test([AFirst|_] = ANodes, [BFirst|_] = BNodes, LeaderA, PortB, Bucket) ->
    ...
    logger:info("Verifying all of the new records are on A"),
    ?assertEqual(0, repl_util:wait_for_reads(AFirst, 1001, 1100, Bucket, 2)),

    logger:info("Pausing 2 seconds"),
    timer:sleep(2000),

    logger:info("Verifying none of the new records are on B"),
...
connect_clusters([AFirst|_] = ANodes, [BFirst|_] = BNodes,
    ...
    logger:info("connect cluster A:~p to B on port ~p", [LeaderA, PortB]),
