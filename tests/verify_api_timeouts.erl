...
confirm() ->
    ...
    HC = rt:httpc(Node),
    logger:info("setting up initial data and loading remote code"),
...
wait_for_error(ReqId) ->
    receive
        {ReqId, done} ->
            logger:error("stream incorrectly finished"),
            error(stream_finished);
        {ReqId, {error, <<"timeout">>}} ->
            logger:info("stream correctly timed out"),
            ok;
        {ReqId, {_Key, _Vals}} ->
            %% the line below is spammy but nice for debugging
            %%{ReqId, {Key, Vals}} ->
            %%logger:info("Got some values ~p, ~p", [Key, Vals]),
            wait_for_error(ReqId);
        {ReqId, Other} ->
...
wait_for_end(ReqId) ->
    receive
        {ReqId, done} ->
            logger:info("stream correctly finished"),
            ok;
        {ReqId, {error, <<"timeout">>}} ->
            logger:error("stream incorrectly timed out"),
