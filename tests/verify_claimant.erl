...
confirm() ->
    ...
    logger:info("Ensure all nodes believe ~p is the claimant", [Node1]),
    [?assertEqual(Node1, claimant_according_to(Node)) || Node <- Nodes],

    %% Stop node1
    logger:info("Stop ~p", [Node1]),
    stop(Node1),
    ?assertEqual(ok, wait_until_unpingable(Node1)),

    %% Ensure all nodes still believe node1 is the claimant
    logger:info("Ensure all nodes still believe ~p is the claimant", [Node1]),
    ...
    logger:info("Mark ~p as down", [Node1]),
    down(Node2, Node1),
    ...
    logger:info("Ensure all nodes now believe ~p is the claimant", [Node2]),
    [?assertEqual(Node2, claimant_according_to(Node)) || Node <- Remaining],

    %% Restart node1 and wait for ring convergence
    logger:info("Restart ~p and wait for ring convergence", [Node1]),
    start(Node1),
    ...
    logger:info("Ensure ~p has rejoined and is no longer down", [Node1]),
    [?assertEqual(valid, status_of_according_to(Node1, Node)) || Node <- Nodes],

    %% Ensure all nodes still believe node2 is the claimant
    logger:info("Ensure all nodes still believe ~p is the claimant", [Node2]),
