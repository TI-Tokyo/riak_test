...
confirm() ->
    ...
    ?LOG_INFO("Mark ~0p as down", [Node1]),
    down(Node2, Node1),
    ...
    ?LOG_INFO("Ensure all nodes now believe ~0p is the claimant", [Node2]),
    [?assertEqual(Node2, claimant_according_to(Node)) || Node <- Remaining],

    %% Restart node1 and wait for ring convergence
    ?LOG_INFO("Restart ~0p and wait for ring convergence", [Node1]),
    start(Node1),
    ...
    ?LOG_INFO("Ensure ~0p has rejoined and is no longer down", [Node1]),
    [?assertEqual(valid, status_of_according_to(Node1, Node)) || Node <- Nodes],

    %% Ensure all nodes still believe node2 is the claimant
    ?LOG_INFO("Ensure all nodes still believe ~0p is the claimant", [Node2]),
