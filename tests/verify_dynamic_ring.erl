...
confirm() ->
    ...
    %% logger:info("testing force-replace during resize"),
    ...
    %% logger:info("Handoff statuses: ~p", [Statuses]),
    ...
    logger:info("submitting resize to subsequently abort. ~p -> ~p", [?SHRUNK_SIZE, ?START_SIZE]),
    submit_resize(?START_SIZE, ANode),
    %% sleep for a second, yes i know this is nasty but we just care that the resize has
    %% made some progress. not really waiting on a condition
    timer:sleep(1000),
    logger:info("aborting resize operation, verifying cluster still has ~p partitions",
               [?SHRUNK_SIZE]),
    rpc:multicall(NewNodes, riak_core_handoff_manager, kill_handoffs, []),
    rt:wait_until_ring_converged(NewNodes),
    abort_resize(ANode),
    rt:wait_until_no_pending_changes(NewNodes),
    logger:info("verifying running vnodes abandoned during aborted resize are shutdown"),
    %% force handoffs so we don't wait on vnode manager tick
    rpc:multicall(Nodes, riak_core_vnode_manager, force_handoffs, []),
    wait_until_extra_vnodes_shutdown(NewNodes),
    logger:info("verifying vnodes abandoned during aborted resize have proxies shutdown"),
    wait_until_extra_proxies_shutdown(NewNodes),
    rt:wait_until_ring_converged(NewNodes),
...
test_resize(CurrentSize, NewSize, ANode, Nodes, {WriteStart,WriteEnd}) ->
    ...
    logger:info("testing ring ~s. ~p -> ~p", [Str, CurrentSize, NewSize]),
    rt:wait_until_ring_converged(Nodes),
...
verify_write_during_resize(Node, Start, End) ->
    receive
        done_writing ->
            logger:info("verifying data written during operation"),
            ?assertEqual([], rt:systest_read(Node, Start, End, ?BUCKET, ?R)),
            ok;
        {errors_writing, Ers} ->
            logger:error("errors were encountered while writing during operation: ~p", [Ers]),
            throw(writes_failed)
    after
        10000 ->
            logger:error("failed to complete writes during operation before timeout"),
