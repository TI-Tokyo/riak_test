...
confirm() ->
    ...
    PBC = rt:pbc(Node1),
    logger:info("Setting last write wins on bucket"),
    B = ?BUCKET,
    ...
    logger:info("Bucket properties ~p", [BProps]),
    K = <<"Key">>,
    V = <<"Value">>,
    Obj = riakc_obj:new(B, K, V),
    logger:info("Writing a simple object"),
    riakc_pb_socket:put(PBC,Obj),
    logger:info("Waiting some time to let the stats update"),
    timer:sleep(10000),
    OrigStats = get_write_stats(Node1),
    logger:info("Stats are now ~p", [OrigStats]),
    Read1 = fun(_N) ->
                    ?assertMatch({ok,_O}, riakc_pb_socket:get(PBC, B, K))
            end,
    logger:info("Repeatedly read that object. There should be no writes"),
    lists:foreach(Read1, lists:seq(1,100)),
    logger:info("Waiting some time to let the stats update"),
    timer:sleep(10000),
    Stats = get_write_stats(Node1),
    logger:info("Stats are now ~p", [Stats]),
