...
confirm() ->
    ...
    logger:info("Join ~p and ~p to ~p", [Node2, Node3, Node1]),
    ...
    logger:info("Ensure that ~p has not yet claimed partitions", [Node2]),
    [?assertEqual([Node1], rt:owners_according_to(Node)) || Node <- Nodes123],

    logger:info("Commit without first printing the plan. This should fail"),
    commit_staged(Node1),

    logger:info("Print staged plan and then commit"),
    print_staged(Node1),
    commit_staged(Node1),

    logger:info("Ensure that ~p now own all partitions", [Nodes123]),
    ?assertEqual(ok, rt:wait_until_nodes_ready(Nodes123)),
    ?assertEqual(ok, rt:wait_until_no_pending_changes(Nodes123)),
    rt:assert_nodes_agree_about_ownership(Nodes123),

    logger:info("Join ~p to the cluster", [Node4]),
    stage_join(Node4, Node1),
    ?assertEqual(ok, rt:wait_until_all_members(Nodes)),

    logger:info("Stage replacement of ~p with ~p", [Node2, Node4]),
    stage_replace(Node1, Node2, Node4),

    logger:info("Print staged plan and commit"),
    print_staged(Node1),
    commit_staged(Node1),

    Nodes134 = [Node1, Node3, Node4],
    logger:info("Ensure that ~p now own all partitions", [Nodes134]),
    ...
    logger:info("Verify that ~p shutdown after being replaced", [Node2]),
    ?assertEqual(ok, rt:wait_until_unpingable(Node2)),

    logger:info("Restart ~p and re-join to cluster", [Node2]),
    rt:start(Node2),
    ...
    logger:info("Schedule force-replace of ~p with ~p", [Node3, Node2]),
    ...
    logger:info("Ensure that ~p now own all partitions", [Nodes124]),
    ...
    logger:info("Stage leave of ~p", [Node2]),
    stage_leave(Node1, Node2),
    logger:info("Stage force-remove of ~p", [Node4]),
    stage_remove(Node1, Node4),

    logger:info("Print staged plan and verify clear_staged works"),
    print_staged(Node1),
    clear_staged(Node1),
    commit_staged(Node1),

    logger:info("Re-stage leave of ~p and force-remove of ~p", [Node2, Node4]),
    stage_leave(Node1, Node2),
    stage_remove(Node1, Node4),
    logger:info("Print staged plan and commit"),
    print_staged(Node1),
    ...
    logger:info("Verify that ~p is the only remaining cluster member", [Node1]),
    ...
    logger:info("Test verify_staged_clustering: Passed"),
    pass.
