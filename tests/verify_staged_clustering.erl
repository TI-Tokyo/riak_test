...
confirm() ->
    ...
    ?LOG_INFO("Join ~0p and ~0p to ~0p", [Node2, Node3, Node1]),
    ...
    ?LOG_INFO("Ensure that ~0p has not yet claimed partitions", [Node2]),
    [?assertEqual([Node1], rt:owners_according_to(Node)) || Node <- Nodes123],

    ?LOG_INFO("Commit without first printing the plan. This should fail"),
    commit_staged(Node1),
    ...
    ?LOG_INFO("Ensure that ~0p now own all partitions", [Nodes123]),
    ?assertEqual(ok, rt:wait_until_nodes_ready(Nodes123)),
    ?assertEqual(ok, rt:wait_until_no_pending_changes(Nodes123)),
    rt:assert_nodes_agree_about_ownership(Nodes123),

    ?LOG_INFO("Join ~0p to the cluster", [Node4]),
    stage_join(Node4, Node1),
    ?assertEqual(ok, rt:wait_until_all_members(Nodes)),

    ?LOG_INFO("Stage replacement of ~0p with ~0p", [Node2, Node4]),
    stage_replace(Node1, Node2, Node4),

    ?LOG_INFO("Print staged plan and commit"),
    print_staged(Node1),
    commit_staged(Node1),

    Nodes134 = [Node1, Node3, Node4],
    ?LOG_INFO("Ensure that ~0p now own all partitions", [Nodes134]),
    ...
    ?LOG_INFO("Verify that ~0p shutdown after being replaced", [Node2]),
    ?assertEqual(ok, rt:wait_until_unpingable(Node2)),

    ?LOG_INFO("Restart ~0p and re-join to cluster", [Node2]),
    rt:start(Node2),
    ...
    ?LOG_INFO("Schedule force-replace of ~0p with ~0p", [Node3, Node2]),
    ...
    ?LOG_INFO("Ensure that ~0p now own all partitions", [Nodes124]),
    ...
    ?LOG_INFO("Stage leave of ~0p", [Node2]),
    stage_leave(Node1, Node2),
    ?LOG_INFO("Stage force-remove of ~0p", [Node4]),
    stage_remove(Node1, Node4),

    ?LOG_INFO("Print staged plan and verify clear_staged works"),
    print_staged(Node1),
    clear_staged(Node1),
    commit_staged(Node1),

    ?LOG_INFO("Re-stage leave of ~0p and force-remove of ~0p", [Node2, Node4]),
    stage_leave(Node1, Node2),
    stage_remove(Node1, Node4),
    ?LOG_INFO("Print staged plan and commit"),
    print_staged(Node1),
    ...
    ?LOG_INFO("Verify that ~0p is the only remaining cluster member", [Node1]),
    ...
    ?LOG_INFO("Test verify_staged_clustering: Passed"),
    pass.
