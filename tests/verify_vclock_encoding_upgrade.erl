...
confirm() ->
    logger:info("Deploying previous cluster"),
    ...
    B = <<"bucket">>,
    V = <<"value">>,
    logger:info("Putting object in previous version node"),
    riakc_pb_socket:put(PrevClient, riakc_obj:new(B, K, V)),
    logger:info("Fetching object from previous version node"),
    {ok, O} = riakc_pb_socket:get(PrevClient, B, K),
    O2 = riakc_obj:update_value(O, <<"value2">>),
    logger:info("Putting updated object in current version node"),
    ok = riakc_pb_socket:put(CurrentClient, O2),
    logger:info("Fetching again from current version node"),
    {ok, O3} = riakc_pb_socket:get(CurrentClient, B, K),
    O4 = riakc_obj:update_value(O3, <<"value2">>),
    logger:info("Putting updated object back in previous version node"),
